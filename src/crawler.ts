...
import {PeerNode, Connection, ConnectionManager, getConfigFromEnv} from "@stellarbeat/js-stellar-node-connector";
...
import {getPublicKeyStringFromBuffer, getIpFromPeerAddress} from "../../node-connector/src/stellar-message-service";
...
export class Crawler {
        ...
        try {
            ...
            if(result && result.data && result.data.core_latest_ledger) {
                if(this.horizonLatestLedger !== result.data.core_latest_ledger){//horizon has a new ledger
                    ...
                    this._logger.warn("[CRAWLER] horizon latest ledger not updated: " + result.data.core_latest_ledger + "Network halted? Trying out next ledger");
                    this._ledgerSequence ++;
                }
            } else {
                this._ledgerSequence ++;
                this._logger.error("[CRAWLER] Could not fetch latest ledger from horizon, using next ledger as fallback " + this._ledgerSequence);
            }
        } catch (e) {
            this._ledgerSequence ++;
            this._logger.error("Error fetching latest ledger from horizon, using next ledger as fallback " + e.message);
        }
        this._logger.info("[CRAWLER] Checking validating states based on latest ledger: " + this._ledgerSequence);
    }
    ...
    async crawl(nodesSeed: Array<Node>, horizonLatestLedger:number = 0): Promise<Array<Node>> {
        ...
        this._pass = 1;
        this._logger.info("[CRAWLER] Starting crawl with seed of " + nodesSeed.length + "nodes.");
    ...
    protected crawlPeerNode(node: PeerNode) {
        if (this._allPeerNodes.has(node.key)) {
            this._logger.debug('[CRAWLER] ' + node.key + ': Node key already used for crawl');
            return;
        }
        this._logger.debug('[CRAWLER] ' + node.key + ': Adding node to crawl queue');
    ...
    processCrawlQueue() {
        while (this._weight < this._maxWeight && this._nodesToCrawl.length > 0) {
            ...
            try {
                ...
                this._logger.debug('[CRAWLER] ' + nextNodeToCrawl.key + ': Start Crawl');

                let connection = this._connectionManager.connect(
                    nextNodeToCrawl.ip,
                ...
                connection.on("data", (stellarMessage:StellarMessage) => {
                    ...
                    if (stellarMessage.switch().value === MessageType.peer().value)
                ...
                this._logger.error('[CRAWLER] ' + nextNodeToCrawl.key + ': Exception: ' + exception.message);
            }
        }
    }
    ...
    wrapUp(node: PeerNode) {
        ...
        this._busyCounter--;
        this._logger.debug("[CRAWLER] Processing " + this._busyCounter + " nodes.");
        ...
        if (this._busyCounter === 0) {//stop the crawler
            ...
            if(this._pass === 1){
                ...
                validatorsToRetry
                    .forEach(validator => {
                        this._logger.info("[CRAWLER] retrying: " + validator.publicKey);
            ...
            if(validatorsToRetry.length === 0){
                this._logger.info("[CRAWLER] Finished with all nodes");
                this._logger.info('[CRAWLER] ' + this._allPeerNodes.size + " nodes crawled of which are active: " + Array.from(this._publicKeyToNodeMap.values()).filter(node => node.active).length);
                this._logger.info('[CRAWLER] of which are validating: '  + Array.from(this._publicKeyToNodeMap.values()).filter(node => node.isValidating).length);
                this._logger.info('[CRAWLER] of which are overloaded: '  + Array.from(this._publicKeyToNodeMap.values()).filter(node => node.overLoaded).length);
                this._logger.info('[CRAWLER] ' + this._nodesThatSuppliedPeerList.size + " supplied us with a peers list.");
    ...
    * CONNECTION EVENT LISTENERS
    ...
    onNodeDisconnected(connection: Connection) {
        try {
            this._logger.info('[CRAWLER] ' + connection.toNode.key + ': Node disconnected');
            if(connection.toNode.publicKey && this._processedValidatingNodes.has(connection.toNode.publicKey)){ //if a node cant complete the handshake, but it is confirmed through other nodes that it is active and validating, we mark it as such.
                ...
                if(node && !node.active) {
                    this._logger.info('[CRAWLER] ' + connection.toNode.key + ': Node did not complete handshake, but is confirmed validating. Marking node as overloaded and validating.');
            ...
            this._logger.error('[CRAWLER] ' + connection.toNode.key + ': Exception: ' + exception.message);
        }
    }

    onHandshakeCompleted(connection: Connection) {
        try {
            this._logger.info('[CRAWLER] ' + connection.toNode.key + ': Handshake succeeded, marking ' + connection.toNode.publicKey + ' as active');
            ...
            if(node.ip !== connection.toNode.ip) {
                this._logger.info('[CRAWLER] ' + connection.toNode.key + ': ' + connection.toNode.publicKey + ' switched IP');
            }
            ...
            if(this._processedValidatingNodes.has(node.publicKey)) { //we already confirmed that the node is validating by listening to externalize messages propagated by other nodes.
                this._logger.info('[CRAWLER] ' + connection.toNode.key + ': ' + connection.toNode.publicKey + ' already confirmed validating, disconnecting');
                node.isValidating = true;
                connection.end();
            } else {
                this.setSCPTimeout(node as any);
                this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': send get scp status message');
    ...
    onPeersReceived(peers: Array<PeerNode>, connection: Connection) {
        try {
            this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': ' + peers.length + ' peers received');
            this._nodesThatSuppliedPeerList.add(connection.toNode);
            peers.forEach(peer => {
                if (!this._allPeerNodes.has(peer.key)) { //newly discovered toNode
                    this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': supplied a newly discovered peer: ' + peer.key);
                    this.crawlPeerNode(peer);
                } else {
                    this._logger.debug('[CRAWLER] peer ' + peer.key + ' already crawled');
                }
            });
        } catch (exception) {
            this._logger.error('[CRAWLER] ' + connection.toNode.key + ': Exception: ' + exception.message);
    ...
    onLoadTooHighReceived(connection: Connection) {
        try {
            this._logger.info('[CRAWLER] ' + connection.toNode.key + ': Load too high message received, disconnected');
            if(connection.toNode.publicKey){
                let node = this._publicKeyToNodeMap.get(connection.toNode.publicKey);
                if(node) {
                    node.active = true;
    ...
    onSCPStatementReceived(connection: Connection, scpStatement: ScpStatement) {
        ...
        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': ' + scpStatement.pledges().switch().name + " message found for node " + publicKey + " for ledger " + scpStatement.slotIndex.toString());

        if (Number(scpStatement.slotIndex) < this._ledgerSequence) {
            return; //older scp messages are ignored.
        }

        let node = this._publicKeyToNodeMap.get(publicKey);
        if(node){
        ...
        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Externalize message found for ledger with sequence ' + scpStatement.slotIndex);
        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': ' + scpStatement.slotIndex + ': ' + scpStatement.nodeId + ': ' + scpStatement.pledges().externalize().commit().value);
        this._processedLedgers.add(Number(scpStatement.slotIndex)); // todo track values

        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': ' + scpStatement.nodeId + ' is validating on ledger:  ' + scpStatement.slotIndex);
        ...
        try {
            this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Detected quorumSetHash: ' + quorumSetHash + ' owned by: ' + quorumSetOwnerPublicKey);
            ...
            if(node){
                if(node.active)//we have successfully connected to node already, so we mark it as validating
                {
                    if(!node.isValidating) { //first time validating is detected
                        this._logger.info('[CRAWLER] ' + node.key + ': Node is validating on ledger:  ' + scpStatement.slotIndex);
                        node.isValidating = true;
                    }
                }
            } else {
                this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Quorumset owner unknown to us, skipping: ' + quorumSetOwnerPublicKey);
                return;
            }

            if (node.quorumSet.hashKey === quorumSetHash) {
                this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Quorumset already known to us for toNode: ' + quorumSetOwnerPublicKey);
                //we don't need any more info for this node, fully processed
                this._processedValidatingNodes.add(node.publicKey); //node is confirmed validating and we have the quorumset. If we connect to it in the future, we can disconnect immediately and mark it as validating.//todo: disconnect if currently connected?
            } else {
                this._logger.info('[CRAWLER] ' + connection.toNode.key + ': Unknown or modified quorumSetHash for toNode, requesting it: ' + quorumSetOwnerPublicKey + ' => ' + quorumSetHash);
                let owners = this._quorumSetHashes.get(quorumSetHash);
                if (owners) {
                    if (owners.has(quorumSetOwnerPublicKey)) {
                        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Already logged quorumSetHash for owner: ' + quorumSetHash + ' owned by: ' + quorumSetOwnerPublicKey);
                    } else {
                        owners.add(quorumSetOwnerPublicKey);
                        this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Logged new owner for quorumSetHash: ' + quorumSetHash + ' owned by: ' + quorumSetOwnerPublicKey);
                    }
                } else {
                    this._quorumSetHashes.set(quorumSetHash, new Set([quorumSetOwnerPublicKey]));
                }
                this._logger.debug('[CRAWLER] ' + connection.toNode.key + ': Requesting quorumset: ' + quorumSetHash);
    ...
    onQuorumSetReceived(connection: Connection, quorumSet: QuorumSet) {
        try {
            this._logger.info('[CRAWLER] ' + connection.toNode.key + ': QuorumSet received: ' + quorumSet.hashKey);
            ...
            owners.forEach(owner => {
                    ...
                    if (nodeWithNewQuorumSet.quorumSet.hashKey === quorumSet.hashKey) {
                        this._logger.debug('[CRAWLER] QuorumSet already updated for toNode: ' + nodeWithNewQuorumSet.publicKey + ' => ' + quorumSet.hashKey);

                    } else {
                        this._logger.debug('[CRAWLER] Updating QuorumSet for toNode: ' + nodeWithNewQuorumSet.publicKey + ' => ' + quorumSet.hashKey);
    ...
    setSCPTimeout(peerNode:PeerNode) {
        this._timeouts.set(peerNode.key, setTimeout(() => {
            this._logger.info('[CRAWLER] ' + peerNode.key + ': SCP Listen timeout reached, disconnecting');
            ...
            if(this._peerNodesParticipatingInSCP.has(peerNode.key) && node && !node.isValidating){ //if two peernodes are reusing the same publicKey, we can't tell which one is (not) validating, because scp messages are based on publickey, not ip.
                ...
                this._logger.info('[CRAWLER] ' + node.key + ': Node was active in SCP, adding more time to listen for externalize messages');
                this.setSCPTimeout(node as any);
            } else {
