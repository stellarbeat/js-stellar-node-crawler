...
class Crawler {
    ...
    constructor(usePublicNetwork = true, durationInMilliseconds = 3000, logger) { //todo report data with event listeners (e.g. connectionmanager)
    ...
    crawlNode(node) {
        ...
        setTimeout(this.crawlNodeDelayed.bind(this), (this._allNodes.size %30)* 2000, node);
                ...
                let nodes = [...this._allNodes.values()].filter(node => node.publicKey === nodePublicKey).forEach( //node could have switched ip, and thus it is now twice in the list with the same public key.
                    nodeWithNewQuorumSet =>
                    {
                        if(nodeWithNewQuorumSet.quorumSet.hashKey === quorumSet.hashKey) {
                            ...
                            console.log(nodeWithNewQuorumSet.key);
